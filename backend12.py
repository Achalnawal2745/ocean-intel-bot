...
# main.py - WITH ORIGINAL COMPLEX ORCHESTRATION
import os
import re
import asyncio
import logging
...
# ==================== SIMPLE ARGO MCP SERVER ====================
class SimpleArgoMCPServer:
    def __init__(self):
        self.db_pool = None
    ...
    # ==================== SINGLE LLM ANALYSIS ====================
    async def llm_analyze_query(self, query: str, context: dict) -> Dict[str, Any]:
        """Single LLM call to analyze query and extract intents + entities"""
        try:
            prompt = f'''
            Analyze this ARGO ocean data query and return JSON with:
            - detected_intents: List of intents from this fixed list: {INTENTS}
            - entities: Extracted parameters like float_id, parameter, region, etc.
            ...
            Query: "temperature of float 2901456"
            {{
                "detected_intents": ["data_query"],
                "entities": {{"float_id": 2901456, "parameter": "temperature"}},
                "confidence": 0.95
            }}
            
            Query: "show me temperature of floats in arabian sea"
            {{
                "detected_intents": ["data_query", "region_data"],
                "entities": {{"region": "arabian_sea", "parameter": "temperature"}},
                "confidence": 0.9
            }}
            ...
            if json_match:
                ...
                result = {"detected_intents": [], "entities": {}, "confidence": 0.0}
            ...
            return {"detected_intents": [], "entities": {}, "confidence": 0.0}
    
    # ==================== SIMPLE ROUTER ====================
    async def process_query_simple(self, query: str, session_id: str) -> Dict[str, Any]:
        """Main processing with simple intent count approach"""
        ...
        llm_analysis = await self.llm_analyze_query(query, context)
        detected_intents = llm_analysis.get("detected_intents", [])
        entities = llm_analysis.get("entities", {})
        confidence = llm_analysis.get("confidence", 0.0)
        
        # Step 2: Simple Intent Count Routing
        intent_count = len(detected_intents)
        
        if intent_count == 0:
            # No intents found → SQL Generation
            logger.info("No intents found → SQL Generation")
            result = await self.sql_generator.generate_sql_response(query, context)
            processing_source = "sql_generation"
            primary_intent = "unknown"
            
        elif intent_count == 1:
            # Single intent → Single Tool Execution
            logger.info(f"Single intent {detected_intents[0]} → Single Tool")
            primary_intent = detected_intents[0]
            result = await self.execute_single_tool(primary_intent, entities)
            processing_source = "single_tool"
            
        else:
            # Multiple intents → ORIGINAL COMPLEX ORCHESTRATION
            logger.info(f"Multiple intents {detected_intents} → Complex Orchestration")
            primary_intent = "complex_orchestration"
            result = await self.process_complex_query(query, context)  # Use original complex orchestration
            processing_source = "complex_orchestration"
        
        # Add to memory
        self.memory.add_exchange(session_id, query, result, primary_intent, entities)
    ...
    # ==================== ORIGINAL COMPLEX ORCHESTRATION (LAYER 4) ====================
    async def process_complex_query(self, query: str, context: dict) -> Dict:
        """ORIGINAL COMPLEX ORCHESTRATION - Layer 4 from original code"""
        try:
            prompt = f"""
    ...
    async def _execute_plan(self, plan: List[Dict]) -> Dict:
        """ORIGINAL: Execute multi-tool plan with dependency resolution"""
        results = {}
    ...
    async def _generate_ai_response(self, original_query: str, tool_results: Dict, expected_output: str) -> Dict:
        """ORIGINAL: Generate AI response from tool results"""
        prompt = f"""
    ...
    # ==================== TOOL EXECUTION ====================
    async def execute_single_tool(self, intent: str, entities: Dict[str, Any]) -> Dict[str, Any]:
        """Execute single tool based on intent"""
        tool_name = INTENT_TO_TOOL_MAP.get(intent)
        
        if not tool_name:
            # Conversational intent - no tool needed
            return self.handle_conversational_intent(intent)
        
        if not hasattr(self, tool_name):
            return {"error": f"Tool {tool_name} not found for intent {intent}"}
        
        try:
            tool_func = getattr(self, tool_name)
            logger.info(f"Executing single tool: {tool_name} with entities: {entities}")
            result = await tool_func(**entities)
            return result
        except Exception as e:
            logger.error(f"Tool execution failed: {e}")
            return {"error": f"Tool execution failed: {str(e)}"}
    
    def handle_conversational_intent(self, intent: str) -> Dict[str, Any]:
        ...
        return {
            "message": responses.get(intent, "I understand your query."),
            "conversational": True
        }

    # ==================== MCP TOOLS (FROM ORIGINAL CODE - KEEP ALL) ====================
    # [ALL THE ORIGINAL TOOLS REMAIN EXACTLY THE SAME]
    # query_measurements, get_float_profile, get_float_trajectory, search_floats_semantic,
    # analyze_with_llm, export_data_ascii, get_floats_in_region, compare_floats,
    # get_temporal_analysis, get_depth_profile, get_trajectory, get_timeseries,
    # get_region_data, list_all_floats, count_floats, search_floats_by_location
...
# [REST OF THE FASTAPI CODE REMAINS EXACTLY THE SAME AS BEFORE]
# Database setup, lifespan, app configuration, and all endpoints remain unchanged

# Global instances
mcp_server = SimpleArgoMCPServer()
...
async def lifespan(app: FastAPI):
    logger.info("Starting ARGO AI Simple System - Intent Count Approach...")
    ...
    # Graceful shutdown
    logger.info("Shutting down ARGO AI Simple System...")
    if hasattr(app.state, 'pool') and app.state.pool:
        await app.state.pool.close()
        logger.info("Database pool terminated successfully")

app = FastAPI(
    title="ARGO AI Simple System - Intent Count Approach",
    description="ARGO oceanographic data system with simple intent count routing",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    return {
        "message": "🌊 ARGO AI Simple System - Intent Count Approach v1.0",
        "status": "ready",
        "features": [
            "✅ Simple Intent Count Routing",
            "✅ Single LLM Analysis", 
            "✅ 1 Intent = Single Tool",
            "✅ 2+ Intents = Complex Orchestration", 
            "✅ 0 Intents = SQL Generation",
...
async def process_query(payload: Dict[str, Any]):
    """Main query endpoint using simple intent count approach"""
    ...
    try:
        result = await mcp_server.process_query_simple(query, session_id)
...
async def get_mcp_tools():
    ...
    tools_info = [
        # Data Retrieval Tools
        {
            ...
            "description": "Query raw measurements with various filters",
            "parameters": ["float_id", "parameter", "depth_range", "cycle_range", "limit"],
            "returns": "Raw measurement data with visualization specs"
        },
        {
            "name": "get_float_profile",
            "description": "Get complete float profile with metadata and measurements",
            "parameters": ["float_id", "cycle_number"],
            "returns": "Float metadata and all measurements"
        },
        {
            "name": "get_depth_profile",
            "description": "Get depth vs parameter profile for frontend plotting",
        ...
        {
            "name": "get_floats_in_region",
            "description": "Get all floats in a specific ocean region",
            "parameters": ["region"],
            "returns": "Float list with regional metadata"
        },
        {
        ...
        {
            "name": "search_floats_by_location",
            "description": "Search for floats near specific coordinates",
            "parameters": ["latitude", "longitude", "radius"],
            "returns": "Nearby floats with distance calculations"
        },
        ...
        },
        {
            ...
            "description": "Perform temporal analysis on float data",
            "parameters": ["float_id", "parameter", "start_date", "end_date"],
            "returns": "Time-based analysis with statistics"
        },
        
        # Search & Discovery Tools
        {
            "name": "search_floats_semantic",
            "description": "Semantic search for floats using ChromaDB",
            "parameters": ["query", "n_results"],
            "returns": "Semantically similar floats and metadata"
        },
        {
            "name": "list_all_floats",
            "description": "List all available floats with metadata",
            "parameters": ["limit", "offset"],
            "returns": "Float list with visualization data"
        },
        {
            "name": "count_floats",
            "description": "Count floats and breakdown by institution/project. Can be filtered by region.",
            "parameters": ["region"],
            "returns": "Statistical counts and summaries"
        },
        
        # Export & AI Tools
        {
            "name": "export_data_ascii",
            "description": "Export ARGO data in ASCII format (CSV/TSV)",
            "parameters": ["float_id", "format_type"],
            "returns": "Formatted data string for download"
        },
        {
            "name": "analyze_with_llm",
            "description": "Direct AI analysis fallback",
            "parameters": ["query", "context_data"],
        ...
        # Complex Processing
        {
            ...
            "description": "AI orchestration for multi-tool complex queries",
            "parameters": ["query", "context"],
            "returns": "Orchestrated multi-tool results with AI synthesis"
        }
    ]
    ...
    return {
        ...
        "tools": tools_info,
        "data_emphasis": "All tools return structured data for frontend visualization"
    }
