...
# main.py - OPTIMIZED AI-FIRST 3-LAYER SYSTEM
import os
import re
import asyncio
import logging
...
INTENTS = [
    ...
    "count_metadata",             # how many floats, count floats
    "profile_path",               # path, trajectory, track, route
    "data_query",                 # profile, temperature, salinity, pressure, depth
    "profile_timeseries",         # timeseries, time series, temporal
    "metadata_details",           # metadata, info, details
    "location_search",            # near, close to, around, latitude, longitude
    "semantic_search",            # similar, like, related, closest
    "profile_comparison",         # compare, comparison, vs, versus, difference
    "region_data",                # region, area, basin
    "data_export",                # download, export, get data
    "map_visualization",          # map, location, where, positions
    "list_floats",                # show floats, list floats, which floats
    "cycle_data"                  # cycle, mission, dive
]

# ==================== INTENT TO TOOL MAPPING ====================
INTENT_TO_TOOL_MAP = {
    "greeting": None,  # No tool needed
    "farewell": None,  # No tool needed
    "data_query": "get_depth_profile",
    "profile_path": "get_trajectory", 
    "profile_timeseries": "get_timeseries",
    "metadata_details": "get_float_profile",
    "location_search": "search_floats_by_location",
    "semantic_search": "search_floats_semantic",
    "profile_comparison": "compare_floats",
    "region_data": "get_floats_in_region",
    "data_export": "export_data_ascii",
    "map_visualization": "get_trajectory",
    "list_floats": "list_all_floats",
    "count_metadata": "count_floats",
    "cycle_data": "get_float_profile"
}
...
class OptimizedArgoMCPServer:
    ...
    # ==================== FIXED LAYER 1: AI DETECTION & TOOL SELECTION ====================
    # This prompt prevents AI from inventing parameters and correctly identifies conversational vs data queries
    ...
    async def layer1_ai_detection(self, query: str, context: dict) -> Dict[str, Any]:
        ...
        try:
            # Define EXACT tool signatures - this is the source of truth
            tool_definitions = {
                "greeting": {
                    "params": [],
                ...
                "compare_floats": {
                    ...
                    "examples": ["compare temperature of floats 2902296 and 2902297"],
                    "returns_data": True,
                    "valid_parameters": ["temperature", "salinity", "pressure", "depth_m"]
                }
            }
    ...
    - Example WRONG: {{"tool": "get_region_data", "parameters": {{"region": "arabian_sea", "parameter": "temperature"}}}}
    - Example RIGHT: {{"tool": "get_region_data", "parameters": {{"region": "arabian_sea"}}}}
    ...
    4. **Regional Queries with Parameters**
    - "temperature of floats in arabian sea" → CANNOT be done with one tool
    - Return null with "requires_multiple_tools": true
    - Reason: get_floats_in_region only returns float IDs, not parameter data
    ...
    Query: "hello"
    → {{"tool": "greeting", "parameters": {{}}, "confidence": 1.0}}

    Query: "what can you do?"
    → {{"tool": "capabilities", "parameters": {{}}, "confidence": 1.0}}

    Query: "show floats in arabian sea"
    → {{"tool": "get_floats_in_region", "parameters": {{"region": "arabian_sea"}}, "confidence": 0.98}}

    Query: "temperature of float 2902296"
    → {{"tool": "get_depth_profile", "parameters": {{"float_id": 2902296, "parameter": "temperature"}}, "confidence": 0.98}}

    Query: "show temperature of floats in arabian sea"
    → {{"tool": null, "reasoning": "Requires multiple tools: get_floats_in_region then get_depth_profile for each", "requires_multiple_tools": true}}

    Query: "compare float 2902296 and 2902297 temperature"
    → {{"tool": "compare_floats", "parameters": {{"float_ids": [2902296, 2902297], "parameter": "temperature"}}, "confidence": 0.95}}

    Query: "path of float 2902296"
    → {{"tool": "get_trajectory", "parameters": {{"float_id": 2902296}}, "confidence": 0.98}}

    ⚠️ REMEMBER:
    - Conversational queries → Use greeting/farewell/capabilities tools (NOT null!)
    - Simple data queries → Select ONE tool with ONLY its defined parameters
    - Complex queries → Return null with requires_multiple_tools: true
    - NEVER invent parameters that don't exist in tool definitions
    ...
    def _validate_layer1_response(self, result: Dict, tool_definitions: Dict) -> Dict:
        ...
        # Check for extra parameters (hallucination)
    ...
    async def layer2_complex_orchestration(self, query: str, context: dict) -> Dict[str, Any]:
        """LAYER 2: AI creates and executes multi-tool plans for complex queries"""
        try:
            prompt = f'''You are a strict JSON generator for ARGO oceanographic data queries. You MUST return ONLY valid JSON, no explanations.

    QUERY: "{query}"
    CONTEXT: {json.dumps(context, default=str)}

    AVAILABLE TOOLS (ONLY USE THESE):
    1. get_floats_in_region(region): Get float IDs in specific ocean regions
       - Valid regions: equator, arabian_sea, indian_ocean, bay_of_bengal, south_atlantic, north_pacific
       - Use ONLY when query asks for floats in a specific region
       - Returns: {{"floats": [2902296, 2902297, ...], "region": "arabian_sea"}}
       
    2. get_depth_profile(float_id, parameter): Get depth profile for ONE specific float
       - Parameters: temperature, salinity, pressure
       - Use ONLY when query asks for depth profile of a specific float
       - Needs: single integer float_id
       
    3. compare_floats(float_ids, parameter): Compare MULTIPLE floats (2 or more)
       - Use ONLY when query asks to compare multiple specific floats
       - Needs: array of float_ids [2902296, 2902297]
       
    4. get_trajectory(float_id): Get trajectory path for ONE specific float
       - Use ONLY when query asks for float movement/trajectory
       - Needs: single integer float_id

    IMPORTANT RULES:
    - If query is NOT about ARGO oceanographic data, return {{"error": "Query not related to ARGO data"}}
    - If query is too vague (like "what is block", "help me", "tell me something"), return {{"error": "Query too vague for ARGO data analysis"}}
    - If query doesn't specify a region, DON'T assume one (like arabian_sea)
    - If query doesn't specify float IDs, DON'T make them up
    - If query doesn't specify parameters (temperature, salinity, pressure), DON'T assume them
    - Only create plans for queries that can be answered with available tools
    - Examples of BAD queries: "what is block", "help", "tell me about ocean", "show me data"
    - Examples of GOOD queries: "temperature of float 2902296", "floats in arabian sea", "compare floats 2902296 and 2902297"

    Create an execution plan. Return ONLY this JSON format (no text before or after):

    {{
        "plan": [
            {{"tool": "tool_name", "parameters": {{"param": "value"}}, "purpose": "Clear purpose"}}
        ],
        "expected_output": "What the plan will produce"
    }}
    ...
    Return JSON now:'''
            ...
            # Check if the plan contains an error
            if "error" in plan_data:
                return {"error": plan_data["error"]}
            
            # Execute the plan
            results = await self._execute_orchestration_plan(plan_data.get("plan", []))
    ...
    async def _execute_orchestration_plan(self, plan: List[Dict]) -> Dict:
        """Execute multi-tool orchestration plan"""
        results = {}
        previous_results = {}
        ...
        for step in plan[:self.execution_limits["max_tools_per_query"]]:
            try:
                ...
                # Replace "FROM_PREVIOUS_RESULT" with actual data
                resolved_params = {}
                for key, value in parameters.items():
                    if value == "FROM_PREVIOUS_RESULT":
                        # Use the last result as input
                        if previous_results:
                            last_tool = list(previous_results.keys())[-1]
                            last_result = previous_results[last_tool]
                            
                            # Smart extraction based on tool and parameter needs
                            if tool_name == "get_depth_profile" and key == "float_id":
                                # get_depth_profile needs single integer float_id
                                if isinstance(last_result, dict) and "floats" in last_result:
                                    if last_result["floats"]:
                                        resolved_params[key] = last_result["floats"][0]  # First float
                                    else:
                                        resolved_params[key] = None
                                else:
                                    resolved_params[key] = last_result
                                    
                            elif tool_name == "compare_floats" and key == "float_ids":
                                # compare_floats needs array of float_ids
                                if isinstance(last_result, dict) and "floats" in last_result:
                                    resolved_params[key] = last_result["floats"]
                                else:
                                    resolved_params[key] = []
                                    
                            elif tool_name == "get_trajectory" and key == "float_id":
                                # get_trajectory needs single integer float_id
                                if isinstance(last_result, dict) and "floats" in last_result:
                                    if last_result["floats"]:
                                        resolved_params[key] = last_result["floats"][0]  # First float
                                    else:
                                ...
                                # For other cases, pass the result as-is
                                resolved_params[key] = last_result
                        else:
                            resolved_params[key] = None
                    else:
                ...
                logger.info(f"Executing orchestration step: {tool_name} for {purpose}")
    ...
    async def process_query_optimized(self, query: str, session_id: str) -> Dict[str, Any]:
        ...
        if tool_name and confidence > 0.7:
            # Layer 1 success - execute single tool
        ...
        # Check if query needs multiple tools
        ...
        # ========== LAYER 2: Complex Orchestration (for unclear cases) ==========
        ...
        responses = {
            ...
            "capabilities": {
                "message": "🎯 I can help you with:\n\n" +
                        ...
                        "• Float trajectories and paths\n" +
                        "• Time series analysis\n\n" +
                        "🗺️ **Regional Analysis:**\n" +
                        "• Find floats in regions (Arabian Sea, Indian Ocean, etc.)\n" +
                        "• Compare floats in different areas\n\n" +
                        "🔍 **Search & Discovery:**\n" +
                        "• Find floats by location coordinates\n" +
                        "• Count and list available floats\n" +
                        "• Compare multiple floats\n\n" +
                        "Just ask naturally! Examples:\n" +
                        "• 'Show temperature of float 2902296'\n" +
                        "• 'List floats in arabian sea'\n" +
                        "• 'Path of float 2902296'",
                "conversational": True,
                "available_tools": [
                    "Temperature/Salinity/Pressure profiles",
                    "Float trajectories",
                    "Regional float lists",
    ...
    # ==================== ALL MCP TOOLS (EXACTLY FROM ORIGINAL) ====================
    ...
    async def export_data_ascii(self, float_id: int, format_type: str = "csv") -> str:
        ...
        try:
            ...
            # Extract records list from query result
    ...
    async def get_depth_profile(self, float_id: int, cycle_number: Optional[int] = None, 
        ...
        # Extract the actual data list from the result
        ...
        async with self.db_pool.acquire() as conn:
            try:
                ...
                if region:
                    ...
                    # Get float_ids in the specified region from the profiles table
...
async def root():
    return {
        ...
        "architecture": [
            ...
            "✅ Tools Handle Validation Naturally"
        ],
...
# Additional endpoints for direct access (same as before)
@app.get("/floats")
