...
# main.py - OPTIMIZED AI-FIRST 3-LAYER SYSTEM WITH FIXED ORCHESTRATION
import os
import re
import asyncio
import logging
...
class OptimizedArgoMCPServer:
    ...
    # ==================== FIXED LAYER 2: AI-POWERED ORCHESTRATION ====================
    async def layer2_complex_orchestration(self, query: str, context: dict) -> Dict[str, Any]:
        """LAYER 2: AI creates and executes multi-tool plans with SMART parameter extraction"""
        try:
            # Enhanced prompt with detailed tool schemas and data flow instructions
            prompt = f'''You are an AI orchestration planner for ARGO oceanographic data. Your task is to create EXECUTABLE multi-step plans.
    ...
    5. get_timeseries(float_id, parameter)
       - Input: float_id (integer), parameter (temperature/salinity/pressure/depth_m)
       - Output: Time series data for ONE float

    🔄 DATA FLOW RULES:
    - Use "EXTRACT:field_name" to specify EXACTLY which field from previous step to use
    - ALWAYS specify the data type and source for each parameter
    - For array outputs (like float IDs), use the ENTIRE array for comparison tools
    - For single outputs, use the first element or specify which one

    🎯 CRITICAL: Your plan MUST be executable. Return ONLY this JSON format:
    ...
    Query: "temperature of floats in arabian sea"
    {{
        "plan": [
            {{
                "tool": "get_floats_in_region",
                "parameters": {{"region": "arabian_sea"}},
                "purpose": "Get all float IDs in Arabian Sea",
                "expected_output_structure": "{{'region': 'arabian_sea', 'floats': [2902296, 2902297], 'float_count': 2}}"
            }},
            {{
                "tool": "compare_floats", 
                "parameters": {{
                    "float_ids": "EXTRACT:floats",  // Use ALL float IDs from previous step
                    "parameter": "temperature"
                }},
                "purpose": "Compare temperature across all Arabian Sea floats",
                "expected_output_structure": "Comparative statistics for all floats"
            }}
        ],
        "expected_output": "Temperature comparison of all Arabian Sea floats",
        "data_flow_explanation": "Step 1 returns float IDs, Step 2 uses ALL those IDs for comparison"
    }}

    Query: "trajectory of first float in indian ocean"
    {{
        "plan": [
            {{
                "tool": "get_floats_in_region",
                "parameters": {{"region": "indian_ocean"}},
                "purpose": "Get float IDs in Indian Ocean",
                "expected_output_structure": "{{'region': 'indian_ocean', 'floats': [2902298, 2902299], 'float_count': 2}}"
            }},
            {{
                "tool": "get_trajectory",
                "parameters": {{
                    "float_id": "EXTRACT:floats[0]"  // Use FIRST float ID from array
                }},
                "purpose": "Get trajectory of the first float",
                "expected_output_structure": "Trajectory data for single float"
            }}
        ],
        "expected_output": "Trajectory path of first Indian Ocean float",
        "data_flow_explanation": "Step 1 returns float IDs array, Step 2 uses first element"
    }}

    Now create a plan for the current query. Return ONLY valid JSON:
    '''
            ...
            # Execute the plan with AI-powered parameter resolution
            results = await self._execute_ai_orchestration_plan(plan_data.get("plan", []))
    ...
    async def _execute_ai_orchestration_plan(self, plan: List[Dict]) -> Dict:
        """Execute multi-tool orchestration plan with AI-powered parameter resolution"""
        results = {}
        previous_results = {}
        ...
        for step_index, step in enumerate(plan[:self.execution_limits["max_tools_per_query"]]):
            try:
                ...
                # AI-POWERED PARAMETER RESOLUTION
    ...
    async def _ai_resolve_parameters(self, step: Dict, previous_results: Dict, step_index: int) -> Dict:
        """AI-powered parameter resolution with validation"""
        tool_name = step['tool']
        parameters = step.get('parameters', {})
        resolved_params = {}
        
        # If this is the first step, just use the provided parameters
        if step_index == 0:
            return parameters
        
        # For subsequent steps, use AI to resolve parameter dependencies
        previous_step_name = list(previous_results.keys())[-1]
        previous_result = previous_results[previous_step_name]
        
        prompt = f'''You are a parameter resolver for ARGO data tools. Resolve parameters for step {step_index + 1}.
    ...
    RESOLUTION RULES:
    - "EXTRACT:field_name" means extract that specific field from previous result
    - "EXTRACT:field_name[index]" means extract element at index from array field
    - For compare_floats with "EXTRACT:floats": use ALL float IDs from array
    - For get_depth_profile with "EXTRACT:floats": use FIRST float ID unless specified
    - Validate data types match tool requirements
