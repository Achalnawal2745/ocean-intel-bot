...
export const DataVisualization: React.FC<DataVisualizationProps> = ({ result: rawResult }) => {
    const { toast } = useToast();

    // Adapt backend response format to component format
    const result = useMemo(() => {
        if (!rawResult) return rawResult;

        // If backend returns plot_data for depth profiles, convert to viz format
        if (rawResult.intent === 'get_depth_profile' && rawResult.plot_data) {
            console.log('🔍 Converting plot_data:', rawResult.plot_data);

            // Extract data from plot_data.data
            let dataArray = rawResult.plot_data.data || [];

            // Handle columnar data format (object of arrays)
            if (dataArray && typeof dataArray === 'object' && !Array.isArray(dataArray)) {
                const keys = Object.keys(dataArray);
                if (keys.length > 0) {
                    const length = Array.isArray(dataArray[keys[0]]) ? dataArray[keys[0]].length : 0;
                    const newArray = [];

                    // Identify keys
                    const depthKey = keys.find(k => /depth/i.test(k)) || 'depth';
                    const valueKey = keys.find(k => /value/i.test(k) || k === rawResult.parameter) || 'value';

                    for (let i = 0; i < length; i++) {
                        const row: Record<string, any> = {};
                        // Normalize keys
                        row['depth_m'] = dataArray[depthKey] && dataArray[depthKey][i];
                        row[rawResult.parameter || 'temperature'] = dataArray[valueKey] && dataArray[valueKey][i];

                        // Also keep original keys just in case
                        keys.forEach(k => {
                            if (k !== depthKey && k !== valueKey) {
                                row[k] = dataArray[k] && dataArray[k][i];
                            }
                        });
                        newArray.push(row);
                    }
            ...
            return {
                ...
                viz: {
                    ...
                    spec: {
                        y: 'depth_m',
                        x: rawResult.parameter || 'temperature',
                        invert_y: true
                    }
                },
                data: dataArray
